...
def time_to_crack_seconds(entropy_bits_val: float,
                          guesses_per_second: float = DEFAULT_GUESSES_PER_SEC) -> float:
    """Estimate time to try all combinations (2^entropy / guesses_per_second)."""
    combinations = 2 ** entropy_bits_val
...
class PasswordReport:
    ...
    def analyze(self) -> Dict[str, Any]:
        ...
        breached = False
        if self.common:
            pwd_hash = sha1_hex(p).lower()
            common_hashes = {sha1_hex(w) for w in self.common}
            breached = pwd_hash in common_hashes
